...
# =====================================================================
# 1. FUNCIÓN PARA GUARDAR EL HISTORIAL DE LOGS
# =====================================================================
...
# =====================================================================
# 2. FUNCIÓN DE EVALUACIÓN ESTADÍSTICA (REORDENADA ARRIBA)
# =====================================================================
def evaluar_paso(error):
    ...
    # Criterio de parada temprana 1: RECHAZO AUTOMÁTICO
    ...
    # Criterio de parada temprana 2: APROBACIÓN ANTICIPADA (50% de la muestra perfecta)
    ...
    # Criterio de parada 3: COMPLETÓ TODA LA MUESTRA SIN SUPERAR ERRORES
    ...
    # Si no se cumple ninguna parada, avanza al siguiente ID aleatorio
    st.session_state.auditorias_realizadas += 1
    st.session_state.llamada_actual = st.session_state.llamadas_disponibles.pop()
    st.rerun()

# =====================================================================
# 3. INICIALIZAR VARIABLES DE SESIÓN (Mantiene el estado de la web)
# =====================================================================
...
# PANTALLA A: Formulario Inicial para ingresar datos del lote
if not st.session_state.validación_activa:
    ...
    if st.button("🚀 Iniciar Proceso de Validación", use_container_width=True):
        if not prompt_name.strip():
            ...
            # Ecuación de población finita (Confianza 95%, Margen error 5%)
            Z = 1.96
            p = 0.05
            e = 0.05
            ...
            # Crear lote mezclado al azar
            llamadas = list(range(1, total_llamadas + 1))
            random.shuffle(llamadas)
            
            # Guardar parámetros iniciales en la sesión
...
# PANTALLA B: Dashboard dinámico con los botones para el auditor
else:
    info = st.session_state.datos_iniciales
    ...
    # Cuadros de métricas arriba de los botones
...
# =====================================================================
# 5. HISTORIAL DE REPORTE Y BOTÓN DE DESCARGA
# =====================================================================
st.write("---")
st.write("### 📊 Historial de Validaciones Registradas")
if os.path.exists(ARCHIVO_LOGS):
    df_logs = pd.read_csv(ARCHIVO_LOGS)
    st.dataframe(df_logs.iloc[::-1], use_container_width=True) # Los registros nuevos salen arriba
